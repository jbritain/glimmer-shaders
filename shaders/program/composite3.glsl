...
void main() {
  ...
  if (isWater) {
    ...
    if (
      ...
      inWater &&
      refract(viewDir, refractionNormal, 1.33) != vec3(0.0)
    ) {
      color.rgb =
        getSky(worldRefractedDir, true) *
        clamp01(smoothstep(13.5 / 15.0, 14.5 / 15.0, skyLightmap));
      vec3 cloudTransmittance;
      vec3 cloudScatter =
        getClouds(vec3(0.0), worldRefractedDir, cloudTransmittance) *
        clamp01(smoothstep(13.5 / 15.0, 14.5 / 15.0, skyLightmap));
    ...
    if (isWater) {
      translucentFeetPlayerPos =
        floor((translucentFeetPlayerPos + cameraPosition) * PIXEL_SIZE) /
          PIXEL_SIZE -
        cameraPosition;
      translucentViewPos = (gbufferModelView *
        vec4(translucentFeetPlayerPos, 1.0)).xyz;
