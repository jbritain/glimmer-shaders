...
vec3 fresnelRoughness(Material material, float NoV){
	if(material.metalID == NO_METAL || material.metalID == OTHER_METAL){
		...
		return material.f0 + (max(vec3(1.0 - material.roughness), material.f0) - material.f0) * pow5(clamp01(1.0 - NoV));
	} else {
		return material.albedo + (max(vec3(1.0 - material.roughness), material.albedo) - material.albedo) * pow5(clamp01(1.0 - NoV));
	}
}
